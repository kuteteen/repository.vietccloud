...
def other_addons():
	...
	if os.path.exists(reposinstaller):
		d = xbmcgui.Dialog().yesno('Add-ons and Repos Installer', 'Installing necessary add-ons and repositories for “Other Addons” section.', '[COLOR magenta]Cài đặt những add-ons và repositories cần thiết cho mục “Other Addons”[/COLOR]', '', '', '')
		if d:
			import time, extract
			dp = xbmcgui.DialogProgress()
			dp.create("Add-ons and Repos Installer", "Working...", "", "")
...
def adult_addons():
	adultsinstaller = xbmc.translatePath(os.path.join(home, 'adults.zip'))
	if os.path.exists(adultsinstaller):
		d = xbmcgui.Dialog().yesno('Adult Add-ons and Repos Installer', 'Installing necessary adult 18+ add-ons and repositories for “Adult Addons” section.', '[COLOR magenta]Cài đặt những add-ons và repositories cần thiết trong mục “Adult Addons” cho người lớn trên 18 tuổi.[/COLOR]', '', '', '')
		if d:
			import time, extract
			dp = xbmcgui.DialogProgress()
			dp.create("Adult Add-ons and Repos Installer", "Working...", "", "")
			addonfolder = xbmc.translatePath(os.path.join('special://', 'home'))
			time.sleep(2)
			dp.update(0,"", "Extracting zip files. Please wait...")
			extract.all(adultsinstaller,addonfolder,dp)
			time.sleep(2)
			os.remove(adultsinstaller)
			xbmcgui.Dialog().ok("Installation Completed.", "Please restart Kodi.", "", "[COLOR magenta]Vui lòng khởi động lại kodi.[/COLOR]")
			sys.exit()
		else:
			sys.exit()
