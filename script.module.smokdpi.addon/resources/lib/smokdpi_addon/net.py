...
"""
    Original Source:
        ...
        script.module.addon.common\lib\addon\common\net.py
        https://github.com/Eldorados/script.module.addon.common/blob/master/lib/addon/common/net.py
        https://github.com/smokdpi/script.module.addon.common/blob/620458c2bf635dc64fe18bb790fdbad8482c7af3/lib/addon/common/net.py
...
class Net:
    ...
    def _set_cloudflare(self, url, challenge, form_data={}, headers={}, compression=True):
        ...
        Args:
            url (str): The URL to site of potentional Cloudflare IUA.

            challenge (str): html contents of the page that raised 503, containing potential Cloudflare IUA Challenge
        Kwargs:
            form_data (dict): A dictionary of form data if passthrough from POST.
    ...
    def url_with_headers(self, url, referer=None, user_agent=None, cookies=None, proxy=None, connection_timeout=None,
        ...
        '''
        Return url with Referer, User-Agent, Cookies, Proxy, Connection-Timout, Encoding, Accept-Charset,
...
class HttpResponse:
    ...
    def __init__(self, response):
        ...
        if not charset:
            charset = re.search('<meta\s+http-equiv=["\']Content-Type["\']\s+content=["\'](?:.+?);' +
        ...
        if charset:
            try:
                # Found Charset: errors='ignore', commit to it and convert to unicode
                html = unicode(html, charset, errors='ignore')
            except:
                pass
        else:
            try:
                # No charset: fail strict
                html = unicode(html, 'utf-8', errors='strict')
            except:
                try:
                    html = unicode(html, 'iso-8859-15', errors='strict')
                except:
                    try:
                        html = unicode(html, 'iso-8859-1', errors='strict')
                    except:
                        pass
